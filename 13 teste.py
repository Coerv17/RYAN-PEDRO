...
def diminuir_estoque():
    ...
    #####    TESTE PEDRO     #####
    Query = 'SELECT MIN(DTVALIDADE),QT FROM FORMULARIO WHERE CODPROD = ?'

    cursor.execute(Query,[produto_id])
    dados = cursor.fetchall()
    dados = dados[0]
    data = dados[0]
    estoque = float(dados[1])

    novaquant = estoque - quantidade 

    Query = 'UPDATE FORMULARIO SET QT = ? WHERE CODPROD = ? AND DTVALIDADE = ?'
    cursor.execute(Query,[novaquant,produto_id,data])
    conn.commit()
    #####      FINAL DO TESTE      #####
